...
import org.javaprotrepticon.android.widgetutils.fragment.base.BaseEntityFragment;
...
public class CustomerFragment extends BaseEntityFragment {
    ...
    private class InitializeEntity extends AsyncTask<Void, Void, Customer> {
    	...
    	public InitializeEntity() {
    		mCustomer = ((CustomerParcel) getArguments().getParcelable("customer")).getCustomer();
    	}
		...
		protected Customer doInBackground(Void... params) {
			...
			try {
				customer = securedStorage.getCustomerDao().queryForId(mCustomer.getId());
